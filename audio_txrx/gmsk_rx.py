...
class gmsk_rx(gr.top_block, Qt.QWidget):
    ...
    def __init__(self):
        ...
        self.settings = Qt.QSettings("GNU Radio", "gmsk_rx")
        ...
        self.rx_freq = rx_freq = 1e9
        self.gmsk_demod = gmsk_demod = (2, 0, 0, 0, 0)
        self.cvsd_decoder = cvsd_decoder = (8, 0.5)
        self.audio_samp_rate = audio_samp_rate = int(400e3)

        ##################################################
        # Blocks
        ##################################################
        self._vol_range = Range(0, 1, 0.01, 0.3, 200)
        self._vol_win = RangeWidget(self._vol_range, self.set_vol, 'Volume', "slider", float)
        self.top_grid_layout.addWidget(self._vol_win, 0, 2, 1, 1)
        for r in range(0, 1):
            self.top_grid_layout.setRowStretch(r, 1)
        for c in range(2, 3):
            self.top_grid_layout.setColumnStretch(c, 1)
        self._rx_gain_range = Range(0, 80, 1, 30, 200)
        ...
        for c in range(1, 2):
            self.top_grid_layout.setColumnStretch(c, 1)
        self.vocoder_cvsd_decode_bf_0 = vocoder.cvsd_decode_bf(cvsd_decoder[0],cvsd_decoder[1])
        ...
        self.digital_gmsk_demod_0 = digital.gmsk_demod(
            samples_per_symbol=gmsk_demod[0],
            gain_mu=gmsk_demod[1],
            mu=gmsk_demod[2],
            omega_relative_limit=gmsk_demod[3],
            freq_error=gmsk_demod[4],
        ...
        self.connect((self.digital_gmsk_demod_0, 0), (self.vocoder_cvsd_decode_bf_0, 0))
    ...
    def get_gmsk_demod(self):
        return self.gmsk_demod

    def set_gmsk_demod(self, gmsk_demod):
        self.gmsk_demod = gmsk_demod

    def get_cvsd_decoder(self):
        return self.cvsd_decoder

    def set_cvsd_decoder(self, cvsd_decoder):
        self.cvsd_decoder = cvsd_decoder
...
def main(top_block_cls=gmsk_rx, options=None):
